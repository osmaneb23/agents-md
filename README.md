...
After PyPI publication, use `pip install agents-md[anthropic]` or skip the install entirely with uvx:
...
Releases go through PyPI Trusted Publishing via `.github/workflows/publish.yml`. To publish: set up a pending publisher for project `agents-md`, repository `osmaneb23/agents-md`, workflow `publish.yml`, environment `pypi`, then tag a GitHub release.
